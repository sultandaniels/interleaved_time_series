...
class Config(object, metaclass=Singleton):
    ...
    mask_only_init = True #only mask the initial segment
    ...
    batch_size = 1 #320 #1024 #512 #8*40 #usually 512 (~35GB) tune this to fit into GPU memory
    acc_grad_batch = 1 #number of batches to accumulate gradients over
    train_data_workers = 1 #2 #set to 1 (check if it changes the speed of the training process)
    test_batch_size = 512
    test_data_workers = 7 #keep at 1

    # Model settings
    model_type = "GPT2" #"llama" #"GPT2" #"transfoXL" #"olmo" #"mamba2"
    ...
    n_layer = 24 #12 #24 #48
    n_head = 12 #8 #12 #18 n_embd % n_head == 0 and n_embd / n_head must be even for RoPE
