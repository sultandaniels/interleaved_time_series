...
def check_valid_cut(positions, new_cut):
    """
    Check if a new cut position is valid.

    Parameters:
    positions (list): List of existing cut positions.
    new_cut (int): The new cut position to check.

    Returns:
    bool: True if the new cut is valid, False otherwise.
    """
    # Check if the new cut is adjacent to or on top of any existing cuts
    for pos in positions:
        if abs(pos - new_cut) < 2:
            return False
    return True

def generate_cut_positions(n_positions, num_cuts):
    positions = []

    valid_cuts = 0
    while valid_cuts < num_cuts:
        # Generate a random position for the cut
        new = np.random.randint(0, n_positions)

        # Check if the position is valid
        if check_valid_cut(positions, new):
            positions.append(new)
            valid_cuts += 1

    print(f"num_cuts: {num_cuts}, valid cuts: {valid_cuts}, positions: {positions}")
    # Add the start and end positions
    if 0 not in positions:
        positions.append(0)
    if n_positions not in positions:
...
def generate_seg_lens(n_positions, sys_in_trace):
    ...
    num_cut = min(rng.poisson(lam), int(n_positions/2)) # number of cuts in the trace. cap it at n_positions/2 since adjacent cuts are no longer allowed.
...
def add_backstories(config, sim_objs, segments, mask_idx, sys_appear, sys_choices, seg_starts, real_seg_lens, test=False):
    ...
    elif (not test) and segments.shape[0] > config.n_positions:
    ...
    # Ensure mask_idx indices are valid for the segments array.
    # In test mode, segments can exceed config.n_positions+1 (no truncation), so bound by segments.shape[0].
    # In train mode, segments are truncated to config.n_positions+1, so both bounds align.
    for idx in mask_idx:
        if idx >= segments.shape[0]:
