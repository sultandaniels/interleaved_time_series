...
from models import GPT2, Mamba2, Llama
...
def preds_thread(config, ckpt_path, make_preds, resume_train, train_conv, logscale, tf, output_dir, ys=None, sim_objs=None, train_mix_dist=False, train_mix_state_dim=False, run_kf_ols=True):
    ...
    if config.model_type == "GPT2":
        ...
        model = Llama.load_from_checkpoint(config.ckpt_path,
                                    n_dims_in=config.n_dims_in,
                                    n_embd=config.n_embd,
                                    n_interm_embd=config.n_interm_embd,
                                    n_layer=config.n_layer,
                                    n_head=config.n_head,
                                    n_dims_out=config.ny,
...
def gen_ckpt_pred_steps(model_name): #change this function to use the model name
    ...
    elif model_name == "ortho_haar_big_mask_backstory_beg" or model_name == "ortho_haar_big_unmask_backstory_beg":
        minval = 1000
        maxval = 316000
        train_int = 1000

        phases = [minval, 10000, 52000, 100000, maxval]

        ckpt_pred_steps = gen_pred_ckpts(minval, maxval, train_int, phases, hande_code_scale=False)
        print(ckpt_pred_steps)
        print(len(ckpt_pred_steps))
        # raise Exception(f"len(ckpt_pred_steps) = {len(ckpt_pred_steps)}. This is not expected. Please check the model name and the parameters.")

    elif model_name == "ortho_haar_big_mask_backstory_plat":
        minval = 5000
        maxval = 121000
        train_int = 1000
    ...
    elif model_name == "ortho_haar_big_mask_backstory_init_mid" or model_name == "ortho_haar_big_mask_backstory_init_mid2":
        minval = 32000
        maxval = 122000
        train_int = 1000
    ...
    elif model_name == "ortho_haar_play_mask_back":
        minval = 100
        maxval = 6000
        train_int = 100
    ...
    elif model_name == "ortho_haar_llama":
        minval = 1000
        maxval = 100000
        train_int = 2000
...
def generate_interleaved_traces(config, ys, sim_objs, num_trials):
    ...
    if config.needle_in_haystack:
        ...
        if config.num_test_traces_configs > 1:
            interleaving += f"_trace_configs_{config.num_test_traces_configs}"
    ...
    # back_frac is also part of the cache key when eval_sys_subset is active
    # (filter_dataset.py:190 computes base = ceil(back_frac * num_tasks)).
    ...
    if not os.path.exists(interleave_traces_dict_path):
        ...
        for ex in range(num_exs):
            ...
            print(f"len(full multi_sys_ys) after ex {ex}:", len(interleave_traces_dict["multi_sys_ys"]))
            end_ex = time.time()
        ...
        # inflate n_positions for iid_gaussian_test even when using cached data
        ...
        # inflate n_positions for backstory_test even when using cached data
...
def set_config_params(config, model_name):
    ...
    if model_name == "gauss":
    ...
    elif model_name == "ortho_haar_big_mask_backstory_beg":
        experiment_name = "backstory_masked_250612_122321.8e31cc_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASKED BACKSTORY MODEL FROM BEGINNING\n\n")
        ...
        experiment_name = "plateau_backstory_masked_250501_221900.f583e5_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASKED BACKSTORY MODEL FROM PLATEAU\n\n")
    ...
    elif model_name == "ortho_haar_big_unmask_backstory_beg":
        experiment_name = "backstory_unmasked_250624_161505.8090a8_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG UNMASKED BACKSTORY FROM BEGINNING MODEL\n\n")
    ...
    elif model_name == "ortho_haar_medium_mask_backstory_beg":
        experiment_name = "backstory_masked_250616_115216.fc25ec_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.584893192461114e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR MEDIUM MASKED BACKSTORY MODEL FROM BEGINNING\n\n")
    ...
    elif model_name == "ortho_haar_small_mask_backstory_beg":
        experiment_name = "backstory_masked_250616_115549.c799d7_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_4.4827548953825996e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR SMALL MASKED BACKSTORY MODEL FROM BEGINNING\n\n")
    ...
    elif model_name == "ortho_haar_big_mask_backstory_no_leak_old":
        ...
        print("\n\nORTHO HAAR BIG MASK BACKSTORY NO LEAK OLD\n\n")
        ...
        experiment_name = "backstory_masked_251209_190520.4ba152_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"
    ...
    elif model_name == "ortho_haar_big_mask_backstory_init_mid":
        experiment_name = "init_250501_221900.f583e5_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASK BACKSTORY INIT SEG FROM MID\n\n")
    ...
    elif model_name == "ortho_haar_big_mask_backstory_init_mid2":
        experiment_name = "init2_250501_221900.f583e5_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASK BACKSTORY INIT SEG FROM MID 2\n\n")
    ...
    elif model_name == "ortho_haar_big_mask_backstory_iid_gaussian":
        experiment_name = "backstory_masked_iid_gaussian_260415_175143.3462af_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASK BACKSTORY IID GAUSSIAN\n\n")
        ...
        config.override("backstory_len", config.ny + 2) #length of the backstory
        config.override("mask_budget", 10) #max # of systems that will be masked on first appearance (alpha)
        config.override("iid_gaussian", True)
        config.override("random_mask", False)
    ...
    elif model_name == "ortho_haar_big_mask_backstory_backlen_1":
        experiment_name = "backstory_masked_back_len_1_260417_191603.1efa47_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO HAAR BIG MASK BACKSTORY BACKSTORY LENGTH 1\n\n")
        ...
        config.override("backstory", True) #use masked backstories
        config.override("init_seg", False) #use masked initial segments
        config.override("backstory_len", 1) #length of the backstory
        config.override("mask_budget", 10) #max # of systems that will be masked on first appearance (alpha)
        config.override("iid_gaussian", False)
        config.override("random_mask", False)
        ...
        experiment_name = "backstory_masked_251217_152442.207d29_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_0.01_num_train_sys_40000"
        ...
        config.override("train_int",1000)  # number of steps between logging (train interval)
        config.override("use_true_len", False)  # Flag for a dataset length to be num_tasks
        config.override("batch_size", 16)  # 2048 #512 #usually 512 (~35GB) tune this to fit into GPU memory
        config.override("train_data_workers", 7)  # set to 1 (check if it changes the speed of the training process)
        config.override("test_batch_size", 512)
        config.override("test_data_workers", 7)  # keep at 1
        ...
        config.override("use_pos_emb", True)  # use positional embeddings
        config.override("n_positions", 250 - config.mask_budget*config.backstory_len if config.mem_suppress and not config.masking else 250)  # 500 for extended OLS #250 #context length
        config.override("n_embd", 5)
        config.override("n_layer", 2)
        config.override("n_head", 1)
        config.override("n_dims_in", int(config.ny + (2 * config.max_sys_trace) + 2) if config.multi_sys_trace else config.ny)  # input dimension is the observation dimension + special token parentheses + special start token + payload identifier
        config.override("n_dims_out", 5)  # (IMPORTANT TO KEEP THIS AT 5 FOR NOW) TODO: this used to be 10 but needs to be fixed to match lin_sys.yaml
        
        config.override("learning_rate", 1e-2)

    elif model_name == "ortho_haar_llama":
        experiment_name = "251217_212211.ccb107_multi_sys_trace_zero_cut_ortho_haar_state_dim_5_ident_C_lr_0.001_num_train_sys_40000"
        ...
        #experiment settings
        config.override("multi_sys_trace", True) #use multiple system traces
        config.override("zero_cut", True) #use zero cut traces for training and validation
        config.override("single_system", False) #use single system traces for testing
        config.override("needle_in_haystack", False) #do not use needle in haystack validation
        ...
        # Model settings
        config.override("model_type", "llama")  # "GPT2" #"transfoXL" #"olmo"
        config.override("use_pos_emb", True)  # use positional embeddings
        config.override("n_positions", 250 - config.mask_budget*config.backstory_len if config.mem_suppress and not config.masking else 250)  # 500 for extended OLS #250 #context length
        config.override("n_embd", 10)
        config.override("n_interm_embd", 10)
        config.override("n_layer", 2)
        config.override("n_head", 5)
        config.override("n_dims_in", int(config.ny + (2 * config.max_sys_trace) + 2) if config.multi_sys_trace else config.ny)  # input dimension is the observation dimension + special token parentheses + special start token + payload identifier
        config.override("n_dims_out", 5)  # (IMPORTANT TO KEEP THIS AT 5 FOR NOW) TODO: this used to be 10 but needs to be fixed to match lin_sys.yaml
        
        config.override("learning_rate", 1e-3)
    ...
    elif model_name == "ortho_haar_big_mask_backstory_back_frac_099975":
        experiment_name = "backstory_masked_back_frac_0.99975_260514_054432.d1a4b5_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000"

        print("\n\nORTHO_HAAR_BIG_MASK_BACKSTORY_BACK_FRAC_099975\n\n")

        # Overrides generated from configs/config.py for backstory_masked_back_frac_0.99975_260514_054432.d1a4b5_multi_sys_trace_ortho_haar_state_dim_5_ident_C_lr_1.4766370475008905e-05_num_train_sys_40000
        config.override("num_tasks", 40000)
        config.override("num_val_tasks", 100)
        config.override("dataset_typ", 'ortho_haar')
        config.override("max_cond_num", 100)
        ...
        config.override("backstory_len", 7)
        config.override("mask_budget", 10)
        config.override("back_frac", 0.99975)
        config.override("multi_sys_trace", True)
        config.override("max_sys_trace", min(25, config.num_tasks))
        config.override("single_system", False)
        config.override("zero_cut", False)
...
def get_test_data(config, experiment_name, num_haystack_ex=50):
    ...
    if (config.datasource == "val"):
        ...
        with open(path + f"train_{config.dataset_typ}{config.C_dist}_state_dim_{config.nx}.pkl", "rb") as f:
            #get train traces
            # samples = pickle.load(f)
            # ys = np.stack(
            #     [entry["obs"][:config.n_positions + 1] for entry in samples], axis=0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--repeat_haystack', help='Boolean. Run experiment that repeats the haystack trace', action='store_true')
    ...
    print("haystack_len arg", args.haystack_len)
    haystack_len = args.haystack_len
    print("plateau arg", args.plateau)
    ...
    print("repeat_haystack arg", args.repeat_haystack)
    repeat_haystack = args.repeat_haystack
    ...
    print("iid_gaussian_test arg", args.iid_gaussian_test)
...
# }
    ...
    if config.plateau:
        print("Running plateau experiment\n\n\n")
    ...
    config.override("repeat_haystack", repeat_haystack)
    if config.repeat_haystack:
        print("Running repeat haystack experiment\n\n\n")
    ...
    config.override("iid_gaussian_test", iid_gaussian_test)
    if config.iid_gaussian_test:
        print("Running iid Gaussian backstories on validation data\n\n\n")

    config.override("backstory_test", backstory_test)
    if config.backstory_test:
        print("Running reverse-dynamics backstories on validation data\n\n\n")
    ...
    if (not (train_conv or multi_haystack)) and (make_preds or saved_preds or resume_train):
        ...
        #     config.override("masking", True)
        # else:
        #     print("Not testing on backstoried data")
        #     config.override("mem_suppress", False)
        #     config.override("backstory", False)
        #     config.override("masking", False)
        ...
        if multi_haystack:
            ...
            for num_sys in num_sys_haystacks:
                ...
                if not make_preds:
                    ...
                    print(f"\ntrain_conv_fin_quartiles_file: {train_conv_fin_quartiles_file}\n")
