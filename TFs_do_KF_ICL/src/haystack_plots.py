...
def plot_needle_position(config, experiment, datasource, state_dim, ckpt_step, valA, valC, haystack_len, steps_in, open_paren_ind, quartiles, seg_ext_quartiles, colors, nope):
    ...
    figure_dir = f"../outputs/GPT2" + ("_NoPE" if nope else "") + f"/{experiment}/figures/multi_sys_trace/needle_in_haystack_examples/{datasource}/" + sys_subset_figure_subdir(config) + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens/" if config.irrelevant_tokens else "") + ("same_tokens/" if config.same_tokens else "") + ("paren_swap/" if config.paren_swap else "") + ("new_hay_insert/" if config.new_hay_insert else "")+ ("identical_haystack/" if config.identical_haystack else "")+ ("repeat_haystack/" if config.repeat_haystack else "")+ (f"len_seg_haystack_{config.len_seg_haystack}/" if config.len_seg_haystack != 10 else "")  + ("fake_out/" if config.fake_out else "")

    os.makedirs(figure_dir, exist_ok=True)
    fig.savefig(figure_dir + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + ("eval_init_" if config.eval_mask_only_init else "") + (f"eval_backlen_{config.eval_backstory_len}_" if config.eval_backstory_len is not None else "") + sys_subset_filename_tag(config) + f"error_ratios_{valA}_embd_dim_{config.n_embd}_state_dim_{state_dim}{valC}_step_{ckpt_step}_haystack_len_{haystack_len}_{timestamp}.pdf", transparent=True)
...
def plot_haystack_train_conv(config, colors, fin_quartiles_ckpt, beg_quartiles_ckpt, x_values, valA, haystack_len, experiment, steps, nope, abs_err=False):
    ...
    if config.nx == 5:
        ...
        with open(f"{os.environ.get('BASE_PATH')}train_and_test_data/ortho_haar/{config.datasource}_irrelevant_tokens_new_hay_insert_pseudo_pred_errs_ortho_haar_ident_C_haystack_len_1.pkl", 'rb') as f:
    ...
    figure_dir = f"../outputs/GPT2" + ("_NoPE" if nope else "") + f"/{experiment}/figures/multi_sys_trace/" + (f"{config.datasource}/" if config.datasource != "val" else "") + sys_subset_figure_subdir(config) + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens/" if config.irrelevant_tokens else "") + ("same_tokens/" if config.same_tokens else "") + ("paren_swap/" if config.paren_swap else "") + ("identical_haystack/" if config.identical_haystack else "") + ("repeat_haystack/" if config.repeat_haystack else "")+ (f"len_seg_haystack_{config.len_seg_haystack}/" if config.len_seg_haystack != 10 else "")   + ("fake_out/" if config.fake_out else "")
    os.makedirs(figure_dir, exist_ok=True)
