...
def plot_needle_position(config, experiment, datasource, state_dim, ckpt_step, valA, valC, haystack_len, steps_in, open_paren_ind, quartiles, seg_ext_quartiles, colors, nope):
    ...
    fig, ax = plt.subplots(num_axes, 1, sharex=True, figsize=(5, 2.5*num_axes)) #
    ...
    figure_dir = f"../outputs/GPT2" + ("_NoPE" if nope else "") + f"/{experiment}/figures/multi_sys_trace/needle_in_haystack_examples/{datasource}/" + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens/" if config.irrelevant_tokens else "") + ("same_tokens/" if config.same_tokens else "") + ("paren_swap/" if config.paren_swap else "") + ("new_hay_insert/" if config.new_hay_insert else "")+ ("identical_haystack/" if config.identical_haystack else "")+ ("repeat_haystack/" if config.repeat_haystack else "")+ (f"len_seg_haystack_{config.len_seg_haystack}/" if config.len_seg_haystack != 10 else "")  + ("fake_out/" if config.fake_out else "")

    os.makedirs(figure_dir, exist_ok=True)
    fig.savefig(figure_dir + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + f"error_ratios_{valA}_embd_dim_{config.n_embd}_state_dim_{state_dim}{valC}_step_{ckpt_step}_haystack_len_{haystack_len}_{timestamp}.pdf", transparent=True)
...
def plot_steps_after_open_token(config, haystack_len, quartiles, seg_ext_quartiles, colors, valA, experiment, datasource, open_paren_ind, n_positions, len_seg_haystack, nope, ckpt_step):
    ...
    needle = 0 #used to be haystack_len - 1
    for key in quartiles.keys():
    ...
    ax.set_ylim(bottom=1e-5, top=2)  # set the y axis limits
    ...
    os.makedirs(figure_dir, exist_ok=True)
    fig.savefig(figure_dir + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + f"last_seg_context_{valA}_embd_dim_{config.n_embd}_step_{ckpt_step}_haystack_len_{haystack_len}_{timestamp}.pdf", transparent=True)
    return None
...
def compute_quartiles_ckpt(config, model_name, steps_in, model_dir, experiment, kal_ckpt, haystack_len, ckpt_steps, train_conv_fin_quartiles_file, train_conv_beg_quartiles_file, x_values_file, abs_err=False):
    ...
    for ckpt_step in ckpt_steps:
        ...
        errs_loc = errs_dir + f"/train_conv_needle_haystack_len_{haystack_len}_{config.datasource}_{config.val_dataset_typ}_state_dim_{config.nx}_" + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("new_hay_insert_" if config.new_hay_insert else "") + ("irrelevant_tokens_" if config.irrelevant_tokens else "") + ("same_tokens_" if config.same_tokens else "")+ ("paren_swap_" if config.paren_swap else "") + ("zero_cut_" if config.zero_cut else "") + (f"len_seg_haystack_{config.len_seg_haystack}" if config.len_seg_haystack != 10 else "") + ("identical_haystack_" if config.identical_haystack else "") + ("repeat_haystack_" if config.repeat_haystack else "") + ("fake_out_" if config.fake_out else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "")
        ...
        if os.path.exists(errs_loc + "err_lss_examples.pkl"):
            ...
            if len(pred_ckpts) == 0:
                #get seg_starts
                seg_starts_per_conf = get_seg_starts_per_config(config)
...
def load_quartiles_ckpt_files(config, haystack_len, model_dir, experiment, abs_err=False):
    ...
    train_conv_fin_quartiles_file = model_dir + experiment + f"/needles/train_conv/" + ("ortho_sync_" if config.val_dataset_typ == "ortho_sync" else "") + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens_" if config.irrelevant_tokens else "") + ("same_tokens_" if config.same_tokens else "")+ ("paren_swap_" if config.paren_swap else "") + ("zero_cut_" if config.zero_cut else "") + ("new_hay_insert_" if config.new_hay_insert else "") + (config.datasource + "_" if config.datasource != "val" else "") + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + ("abs_err_" if abs_err else "") + (f"len_seg_haystack_{config.len_seg_haystack}" if config.len_seg_haystack != 10 else "")+ ("identical_haystack_" if config.identical_haystack else "") + ("repeat_haystack_" if config.repeat_haystack else "") + ("fake_out_" if config.fake_out else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + f"train_conv_fin_quartiles_haystack_len_{haystack_len}.pkl"

    train_conv_beg_quartiles_file = model_dir + experiment + f"/needles/train_conv/" + ("ortho_sync_" if config.val_dataset_typ == "ortho_sync" else "")  + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens_" if config.irrelevant_tokens else "") + ("same_tokens_" if config.same_tokens else "")+ ("paren_swap_" if config.paren_swap else "") + ("zero_cut_" if config.zero_cut else "") + ("new_hay_insert_" if config.new_hay_insert else "") + (config.datasource + "_" if config.datasource != "val" else "") + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + ("abs_err_" if abs_err else "") + (f"len_seg_haystack_{config.len_seg_haystack}" if config.len_seg_haystack != 10 else "")+ ("identical_haystack_" if config.identical_haystack else "") + ("repeat_haystack_" if config.repeat_haystack else "") + ("fake_out_" if config.fake_out else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + f"train_conv_beg_quartiles_haystack_len_{haystack_len}.pkl"

    x_values_file = model_dir + experiment + f"/needles/train_conv/" + ("ortho_sync_" if config.val_dataset_typ == "ortho_sync" else "") + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens_" if config.irrelevant_tokens else "") + ("same_tokens_" if config.same_tokens else "")+ ("paren_swap_" if config.paren_swap else "") + ("zero_cut_" if config.zero_cut else "") + ("new_hay_insert_" if config.new_hay_insert else "")  + (config.datasource + "_" if config.datasource != "val" else "") + (f"late_start_{config.late_start}_" if config.late_start is not None else "") + (f"len_seg_haystack_{config.len_seg_haystack}" if config.len_seg_haystack != 10 else "")+ ("abs_err_" if abs_err else "") + ("fake_out_" if config.fake_out else "")+ ("identical_haystack_" if config.identical_haystack else "") + ("repeat_haystack_" if config.repeat_haystack else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "") + (f"x_values_haystack_len_{haystack_len}.npy")
    ...
    #     raise NotImplementedError("Check the x_values, fin_quartiles_ckpt, beg_quartiles_ckpt")
...
def plot_haystack_train_conv(config, colors, fin_quartiles_ckpt, beg_quartiles_ckpt, x_values, valA, haystack_len, experiment, steps, nope, abs_err=False):
    ...
    for key in fin_quartiles_ckpt.keys():
        ...
        if key == "MOP": #key == "OLS_analytical_ir_1" or key == "OLS_ir_1": #key == "MOP" or 
            ...
            for step in steps:
                ...
                #             yerr=[pseudo_pred_qs[1, 1 + step] - pseudo_pred_qs[0, 1 + step], 
                #                   pseudo_pred_qs[2, 1 + step] - pseudo_pred_qs[1, 1 + step]],
                #             fmt='o', color=colors[col_count],
                #             label=f"Pseudoinv: {step}{final_lab_suffix}", capsize=2, markersize=5, marker=".", linestyle=":", linewidth=2, zorder=0)
                # ax_len.errorbar(x_values[::skip], pseudo_pred_qs[1, 1 + step],
                #             yerr=[pseudo_pred_qs[1, 1 + step] - pseudo_pred_qs[0, 1 + step], 
                #                   pseudo_pred_qs[2, 1 + step] - pseudo_pred_qs[1, 1 + step]],
                #             fmt='o', color=colors[col_count],
                #             label=f"Pseudoinv: {step}{final_lab_suffix}", capsize=2, markersize=5, marker=".", linestyle=":", linewidth=2, zorder=0)

                # ax.hlines(y = pseudo_pred_qs[1, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label=f"Pseudoinv: {step}{beg_lab_suffix}")
                # # ax.hlines(y = pseudo_pred_qs[0, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label='__no_legend__')
                # # ax.hlines(y = pseudo_pred_qs[2, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label='__no_legend__')
                # ax.fill_between(x_values, pseudo_pred_qs[0, 1 + step], pseudo_pred_qs[2, 1 + step], alpha=0.05, color=colors[col_count])

                # ax_len.hlines(y = pseudo_pred_qs[1, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label=f"Pseudoinv: {step}{beg_lab_suffix}")
                # # ax_len.hlines(y = pseudo_pred_qs[0, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label='__no_legend__')
                # # ax_len.hlines(y = pseudo_pred_qs[2, 1 + step], xmin=x_values[0], xmax=x_values[-1], color=colors[col_count], linestyle=':', linewidth=2, label='__no_legend__')
                # ax_len.fill_between(x_values, pseudo_pred_qs[0, 1 + step], pseudo_pred_qs[2, 1 + step], alpha=0.05, color=colors[col_count])
    ...
    figure_dir = f"../outputs/GPT2" + ("_NoPE" if nope else "") + f"/{experiment}/figures/multi_sys_trace/" + (f"{config.datasource}/" if config.datasource != "val" else "") + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens/" if config.irrelevant_tokens else "") + ("same_tokens/" if config.same_tokens else "") + ("paren_swap/" if config.paren_swap else "") + ("identical_haystack/" if config.identical_haystack else "") + ("repeat_haystack/" if config.repeat_haystack else "")+ (f"len_seg_haystack_{config.len_seg_haystack}/" if config.len_seg_haystack != 10 else "")   + ("fake_out/" if config.fake_out else "")
    os.makedirs(figure_dir, exist_ok=True)
...
def haystack_plots_needle_full(config, haystack_len, output_dir, ckpt_step, steps_in, colors, compute_more=False):
    ...
    if ckpt_step is not None:
        ...
        if quartiles is None or seg_ext_quartiles is None or compute_more:
            ...
            errs_dir = model_dir + experiment + f"/prediction_errors{config.C_dist}_step={ckpt_step}.ckpt"
            errs_loc = errs_dir + f"/needle_haystack_len_{config.num_sys_haystack}_{config.datasource}_" + f"{config.val_dataset_typ}_state_dim_{config.nx}_"  + ("fix_needle_" if config.fix_needle else "") + ("opposite_ortho_" if config.opposite_ortho else "") + ("irrelevant_tokens_" if config.irrelevant_tokens else "") + ("same_tokens_" if config.same_tokens else "")+ ("paren_swap_" if config.paren_swap else "") + ("zero_cut_" if config.zero_cut else "") + ("new_hay_insert_" if config.new_hay_insert else "") + ("identical_haystack_" if config.identical_haystack else "") + ("repeat_haystack_" if config.repeat_haystack else "") + ("fake_out_" if config.fake_out else "") + ("iid_gaussian_test_" if config.iid_gaussian_test else "") + ("backstory_test_" if config.backstory_test else "")
